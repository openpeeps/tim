...
import ../src/tim/engine/transpilers/rbgen
...
proc transpile(code: string): string =
  ...
  initParser(code)
  initLibrary()
  var cg = rbgen.initCodeGen(script, module, mainChunk)
  result = $cg.genScript(program, none(string), isMainScript = true)

proc runRuby(code: string): string =
  let tmpFile = getTempDir() / "tim_test.rb"
  writeFile(tmpFile, code)
  let sts = execCmdEx("ruby " & tmpFile)
  if sts.exitCode != 0:
    raise newException(CatchableError, "ruby exited with code " & $sts.exitCode & ": " & sts.output)
  sts.output.strip()

template checkOutput(transpiled, expected: string) =
  let wrapper = "puts HelloWorld.render({}, {})\n"
  let ruby = transpiled & "\n" & wrapper
  let output = runRuby(ruby)
  check output == expected

suite "RubyGen — Transpiler":
