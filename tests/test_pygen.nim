...
import ../src/tim/engine/transpilers/pygen
...
proc transpile(code: string): string =
  ...
  initParser(code)
  initLibrary()
  var cg = pygen.initCodeGen(script, module, mainChunk)
  result = $cg.genScript(program, none(string), isMainScript = true)

proc runPy(code: string): string =
  let tmpFile = getTempDir() / "tim_test.py"
  writeFile(tmpFile, code)
  let sts = execCmdEx("python3 " & tmpFile)
  if sts.exitCode != 0:
    raise newException(CatchableError, "python3 exited with code " & $sts.exitCode & ": " & sts.output)
  sts.output.strip()

template checkOutput(transpiled, expected: string) =
  let wrapper = "print(HelloWorld.render({}, {}))\n"
  let pyCode = transpiled & "\n" & wrapper
  let output = runPy(pyCode)
  check output == expected

suite "PythonGen — Transpiler":
...
""")
  ...
  test "empty template":
    let code = transpile("")
    check code.contains("class HelloWorld")
